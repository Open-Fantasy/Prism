...
export class Prism {
  ...
  private logicTick(prism: Prism, delta: number) {
    ...
    if (prism.settings.gatherStats) prism.runtimeStats.updateLogic(delta);
    if (prism.settings.enableNetwork) prism.network.update(delta);
  ...
  private renderTick(prism: Prism, delta: number) {
    ...
    if (prism.settings.gatherStats) prism.runtimeStats.updateRender(delta);
