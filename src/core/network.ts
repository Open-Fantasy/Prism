...
export class Network {
  ...
  start(serverAddr: string) {
    if (this.networkStatus != NetworkStatus.waitingForInitialization) console.log('Network already initialized');
  ...
  update(delta: number): void {
    if (this.networkStatus != NetworkStatus.connected) return;

    if (this._messageQueue.length == 0 && this._priorityMessageQueue.length == 0) return;
