...
export class Renderer {
  ...
  private clickEvent(hover: MouseEvent) {
    ...
    if (intersects.length != 0) {
      ...
      const clicked = intersects[0].object as ClickableUIElement;
  ...
  private hoverEvent(click: MouseEvent) {
    ...
    if (intersects.length == 0) {
      if (this.hoveredUiObj?.offHover) this.hoveredUiObj.offHover();
      this.hoveredUiObj = undefined;
      return;
    }
    if (intersects.length != 0) {
      const hovered = intersects[0].object as BaseUIElement;
      if (this.hoveredUiObj === hovered) return; // nothing happens, same object as before
      // new hover
      if (this.hoveredUiObj?.offHover) this.hoveredUiObj.offHover();
